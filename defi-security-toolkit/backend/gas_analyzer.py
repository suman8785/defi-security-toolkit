...
class GasAnalyzer:
    ...
    def _analyze_storage_patterns(self, content: str) -> Dict[str, Any]:
        ...
        storage_reads = len(re.findall(r'\b\w+
$$
.*?
$$', content))
        storage_writes = len(re.findall(r'\b\w+
$$
.*?
$$\s*=', content))
    ...
    def _analyze_loops(self, content: str) -> List[Dict[str, Any]]:
        ...
        # Find for loops
        for_loops = re.findall(r'for\s*$[^)]+$\s*{([^}]+)}', content)
        ...
        # Find while loops
        while_loops = re.findall(r'while\s*$[^)]+$\s*{([^}]+)}', content)
    ...
    def _estimate_function_gas(self, func_body: str) -> int:
        ...
        gas += len(re.findall(r'\w+
$$
', func_body)) * self.GAS_COSTS['SLOAD']
        gas += len(re.findall(r'
$$\s*=', func_body)) * self.GAS_COSTS['SSTORE']
